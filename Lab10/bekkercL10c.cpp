...
bool AskQuestion(int N, string Q[], string A[], string B[], string C[], string D[], char R[]);
...

{
  ...
  for (int q=1; q <=5; q++)
    ...
    {
      if (AskQuestion(number, Questions, aAnswers, bAnswers, cAnswers, dAnswers, Right))
...
bool AskQuestion(int N,string Q[], string A[], string B[], string C[], string D[], char R[])
{
 ...
 // Calculate a random number between 0 and the number of questions
 Qnum = rand() % N;
